...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/campaigns/${id}`)
      .then((r) => {
        if (r.status === 404) throw new Error(t("campaignNotFound"));
    ...
    return (
      <div className="min-h-screen bg-zinc-50 p-4 dark:bg-zinc-950 md:p-6">
        <div className="mx-auto max-w-5xl">
          <p className="text-zinc-500 dark:text-zinc-400">{t("loadingCampaign")}</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !campaign) {
    return (
      <div className="min-h-screen bg-zinc-50 p-4 dark:bg-zinc-950 md:p-6">
        <div className="mx-auto max-w-5xl">
          <p className="text-red-600 dark:text-red-400">{error || t("campaignNotFound")}</p>
        </div>
      </div>
    );
  }
  ...
  const dateStr =
    startDate && endDate
      ...
      : startDate
        ? t("fromDate", { date: new Date(startDate).toLocaleDateString() })
        : endDate
          ? t("untilDate", { date: new Date(endDate).toLocaleDateString() })
          : t("noDatesSet");
    ...
    <div className="min-h-screen bg-zinc-50 p-4 dark:bg-zinc-950 md:p-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: t("totalSessions"), value: metrics.totalSessions },
            { label: t("completionRate"), value: `${metrics.completionRate}%` },
            { label: t("avgAiScore"), value: metrics.avgAiScore ?? tCommon("notAvailable") },
            { label: t("avgHumanScore"), value: metrics.avgHumanScore ?? tCommon("notAvailable") },
          ].map((m) => (
            <div
              key={m.label}
        ...
        {Object.keys(recommendations).length > 0 && (
          <div className="mb-6 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 md:p-6">
            <h2 className="mb-4 text-lg font-semibold text-zinc-900 dark:text-zinc-50">{t("aiRecommendationDistribution")}</h2>
        ...
        {topCandidates.length > 0 && (
          <div className="mb-6 rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 md:p-6">
            <h2 className="mb-4 text-lg font-semibold text-zinc-900 dark:text-zinc-50">{t("topCandidates")}</h2>
              ...
              <table className="w-full text-sm">
                <thead className="bg-zinc-50 dark:bg-zinc-800">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-medium text-zinc-700 dark:text-zinc-300">{t("aiScore")}</th>
                    <th className="px-4 py-3 text-left font-medium text-zinc-700 dark:text-zinc-300">{t("humanScore")}</th>
                ...
                <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                  {topCandidates.map((tc) => (
                    <tr key={tc.sessionId}>
                      ...
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{tc.aiAvg?.toFixed(1) ?? tCommon("notAvailable")}</td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{tc.humanAvg?.toFixed(1) ?? tCommon("notAvailable")}</td>
                      <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{tc.recommendation || tCommon("notAvailable")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {topCandidates.map((tc) => (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">{tCommon("ai")}:</span>{" "}
                      {tc.aiAvg?.toFixed(1) ?? tCommon("notAvailable")}
                    </div>
                    <div>
                      <span className="text-zinc-500 dark:text-zinc-400">{tCommon("human")}:</span>{" "}
                      {tc.humanAvg?.toFixed(1) ?? tCommon("notAvailable")}
                    </div>
                  </div>
                  <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                    {tc.recommendation || tCommon("notAvailable")}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 md:p-6">
          <h2 className="mb-4 text-lg font-semibold text-zinc-900 dark:text-zinc-50">{t("positions")}</h2>
          {positions.length === 0 ? (
            <p className="text-sm text-zinc-500 dark:text-zinc-400">{t("noPositionsInCampaign")}</p>
          ) : (
            <>
              <div className="hidden overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 md:block">
                <table className="w-full text-sm">
                  <thead className="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                      <th className="px-4 py-3 text-left font-medium text-zinc-700 dark:text-zinc-300">{tPositions("titleLabel")}</th>
                      <th className="px-4 py-3 text-left font-medium text-zinc-700 dark:text-zinc-300">{tPositions("level")}</th>
                      <th className="px-4 py-3 text-left font-medium text-zinc-700 dark:text-zinc-300">{tPositions("requirements")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {positions.map((p) => (
