...
export default function TranscriptPage() {
  ...
  async function startEvaluationJob() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || t("evaluationFailed"));
      }
      ...
      setEvalJob({
        phase: "failed",
        error: err instanceof Error ? err.message : tCommon("unknown"),
      });
    }
  }
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!res.ok) throw new Error(t("evaluationFailed"));
        ...
        if (job.status === "completed") {
          ...
          setEvalJob({
            phase: "failed",
            error: job.error || t("evaluationFailed"),
          });
        }
  ...
  async function deleteVersion(versionId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || tCommon("unknown"));
      }
  ...
  function RecommendationBadge({ rec }: { rec: string | null }) {
    ...
    return (
      <span className={`rounded-lg px-3 py-1 text-sm font-medium ${colors[rec] || colors.maybe}`}>
        {labelMap[rec] || rec.replace("_", " ").toUpperCase()}
      </span>
    );
  }
  ...
  if (!sessionData) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 p-4 dark:bg-zinc-950">
        <p className="text-red-600 dark:text-red-400">{t("sessionNotFound")}</p>
      </div>
    );
  }
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50 sm:text-2xl">
              {candidate?.name || tCommon("unknown")} — {position?.title || t("interviewTranscript")}
            </h1>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              {position?.level} · {session.status === "completed" ? t("completed") : t("inProgress")} ·{" "}
              {session.currentTurn}/{session.maxTurns} {t("turns")}
                  ...
                  <div key={msg.id} className="border-l-2 border-zinc-200 dark:border-zinc-700 pl-4">
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
                        {msg.role === "interviewer" ? t("interviewerLabel") : t("candidateLabel")}
                      </p>
                      {msg.role === "interviewer" && (
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            title={speakingMsgId === msg.id ? t("stop") : t("speak")}
                          >
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-50">
                  {isViewingHistory ? t("evaluationHistorical") : t("aiEvaluation")}
                </h2>
                ...
                <div>
                  ...
                  {evalJob.phase === "polling" && (
                    <div className="mb-3 flex items-center gap-2">
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-300 border-t-zinc-900 dark:border-zinc-700 dark:border-t-zinc-50" />
                      <p className="text-sm text-zinc-600 dark:text-zinc-400">
                        {t("evaluatingJob", { id: evalJob.jobId?.slice(0, 8) || "" })}
                      </p>
                    </div>
                  )}
                  {evalJob.phase === "failed" && (
                    <div className="mb-3 rounded-lg bg-red-50 p-2 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-300 border border-red-200 dark:border-red-800">
                      <p className="font-medium">{t("evaluationFailed")}</p>
              ...
              {session.status !== "completed" && (
                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                  {t("evaluationAvailableAfterComplete")}
                </p>
              )}
              ...
              {activeEvaluation && (
                <div className="space-y-3">
                  ...
                  <StarRating
                    ...
                    label={t("relevanceToRole")}
                  />

                  <div className="pt-2 border-t border-zinc-200 dark:border-zinc-700">
                    <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">{t("aiRecommendationLabel")}</p>
                    ...
                    <div className="pt-3 border-t border-zinc-200 dark:border-zinc-700">
                      ...
                      <div className="mt-2">
                        ...
                        >
                          ...
                          <option value="strong_yes">{t("strongYes")}</option>
                          <option value="yes">{t("yes")}</option>
                          <option value="maybe">{t("maybe")}</option>
                          <option value="no">{t("no")}</option>
                          <option value="strong_no">{t("strongNo")}</option>
                        </select>
                      </div>
                    </div>
                  )}
                  ...
                  {!isViewingHistory && (
                    <div className="pt-3 border-t border-zinc-200 dark:border-zinc-700">
                      ...
                      >
                        {savingCalibration ? t("saving") : t("saveCalibration")}
                      </button>
                    </div>
                  )}
                    ...
                    <div className="pt-3 border-t border-zinc-200 dark:border-zinc-700">
                      ...
                      {evalJob.phase === "posting" && (
                        <div className="mb-2 flex items-center gap-2">
                          <div className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-300 border-t-zinc-900 dark:border-zinc-700 dark:border-t-zinc-50" />
                          <p className="text-sm text-zinc-600 dark:text-zinc-400">{t("startingEvaluation")}</p>
                        </div>
                      )}
                      {evalJob.phase === "polling" && (
                        <div className="mb-2 flex items-center gap-2">
                          <div className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-300 border-t-zinc-900 dark:border-zinc-700 dark:border-t-zinc-50" />
                          <p className="text-sm text-zinc-600 dark:text-zinc-400">{t("evaluatingJob", { id: evalJob.jobId?.slice(0, 8) || "" })}</p>
                        </div>
                      )}
                      {evalJob.phase === "failed" && (
